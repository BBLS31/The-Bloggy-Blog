...
const Admin = () => {
  ...
  useEffect(() => {
    fetch("https://the-bloggy-blog-api.onrender.com/users", {
      credentials: "include",
    })
  ...
  useEffect(() => {
    if (selectedUser) {
      ...
      fetch(`https://the-bloggy-blog-api.onrender.com/user-blogs/${selectedUser}`, {
        credentials: "include",
      })
        .then((response) => response.json())
              ...
              {blogs.map((blog) => (
                <div key={blog._id}>
                  ...
                  <div className="ai">
                    <img
                      className="admin-img"
                      src={`https://the-bloggy-blog-api.onrender.com/${blog.cover}`}
                      alt={blog.title}
                    />
                  </div>
                  <p
